...
#  - unsafe_hash: Force the addition of a .__hash__() method?
# (Default is False.)
#  - frozen: If True, assigning to fields raise an exception.
# (Default is False.)

# def __repr__(self):
# 	suit_to_str = self.suit if self.suit == '♣' or
#           self.suit == '♠' else chr(ord(self.suit) - 4)
# 	return suit_to_str + self.rank


#  In general, a Python object has two different string representations:
#  - repr(obj) is defined by obj.__repr__() and should return a
#  developer-friendly representation of obj.
# 	If possible, this should be a code that can recreate obj.
#  Data classes do this.
#  - str(obj) is defined by obj.__str__() and should return a user-friendly
#  representation of obj.
# Data classes do not implement a .__str__() method, so Python will fall
# back to the .__repr__() method.
...
class PlayingCard:
    ...
    def __post_init__(self):  # XXX no funciona!!
        if self.rank not in PlayingCard.RANKS or
                self.suit not in PlayingCard.SUITS:
            self.sort_index = -1
        else:
            self.sort_index = (PlayingCard.RANKS.index(
                self.rank) * len(PlayingCard.SUITS) \
